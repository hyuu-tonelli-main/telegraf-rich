...
bot.command('brat', async (ctx) => {  
  try {  
    ...
    // 1. Kalau kosong, kasih tau formatnya  
    if (!text) {  
      return ctx.reply("✍️ Kirim teksnya juga.\n\nContoh:\n/brat halo semua");  
    }  
  
    // 2. Kasih tau lagi diproses  
    await ctx.reply("⏳ Sedang membuat stiker brat...");  
  
    // 3. Panggil API brat, ambil gambarnya sebagai buffer  
    // CATATAN: endpoint contoh, verifikasi masih hidup / ganti kalau mati  
    const { data } = await axios.get(  
      `https://brat.siputzx.my.id/?text=${encodeURIComponent(text)}`,  
      { responseType: "arraybuffer" }  
    );  
  
    const buffer = Buffer.from(data);  
  
    // 4. Kirim sebagai stiker (Telegram butuh format webp buat sticker)  
    await ctx.replyWithSticker({ source: buffer });  
  
    // Kalau mau dikirim sebagai foto biasa aja, ganti baris di atas jadi:  
    // await ctx.replyWithPhoto({ source: buffer });  
  
  } catch (err) {  
    console.error(err);  
    ctx.reply("⚠️ Gagal bikin brat, coba lagi.");  
  }  
});
